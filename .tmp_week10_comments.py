...
# keep existing grades, replace comments only
grades = {}
...
for fp in sorted(sub_dir.iterdir()):
    ...
    # if multiple, keep longer (usually fuller)
...
with grade_csv.open('w', newline='', encoding='utf-8') as f:
    w = csv.writer(f)
    w.writerow(["Student", "Grade", "Comments"])
    w.writerows(rows)

print(f"Updated individualized comments for {len(rows)} students in {grade_csv}")
for s,g,c in rows:
    print(s, g, "|", c[:95])
