...
def q_score(t: str, q: int):
    ...
    own_pts, _, wc = own_lang(t)

    score = attempt_pts + req_pts + acc_pts + own_pts
    if wc < 90:
        score -= 2

    return int(round(max(0, min(25, score))))
...
for fp in sorted(sub_dir.iterdir()):
    ...
    score = q_score(read_text(fp).lower(), q)
    prev = by_student[stu].get(q)
    if prev is None or score > prev['score']:
        by_student[stu][q] = {'score': score, 'ext': fp.suffix.lower()}
...
for stu in sorted(by_student):
    ...
    comment = "Strengths: submitted substantive work across exam sections"
...
print(f"Updated {out_csv} with sub-scores")
