...
### Visualizing Tree Diversity: Six Trees from the Same Forest

Below we extract six individual trees from a trained Random Forest and plot them. Notice how each tree splits on **different features** and **different thresholds** — yet together they form a powerful ensemble.
...
for (tree_id in c(1, 5, 15, 30, 60, 100)) {
  ...
  # Plot as a simple rpart-style tree using rpart on the bootstrap sample
  ...
  if (length(unique(train_2f$Species[boot_obs])) > 1 && length(boot_obs) > 5) {
    t_i <- rpart(Species ~ .,
                 ...
                 control = rpart.control(maxdepth = 3, minsplit = 2, cp = 0))
    rpart.plot(t_i,
...
# Get numeric predictions (1/2/3) for each tree via votes
get_tree_preds <- function(rf_obj, newdata) {
  # Use individual tree predictions via predict(..., predict.all=TRUE)
  p <- predict(rf_obj, newdata, predict.all = TRUE)
  p$individual  # matrix: nrow = observations, ncol = ntree
}

bag_preds_mat <- get_tree_preds(rf_bag, iris_test)
rf_preds_mat  <- get_tree_preds(rf_rf,  iris_test)

# Pairwise correlation between tree prediction vectors
compute_pair_corr <- function(pred_mat) {
  n <- ncol(pred_mat)
  cors <- matrix(NA, n, n)
  for (i in 1:n) for (j in 1:n) {
    cors[i,j] <- cor(as.numeric(pred_mat[,i]), as.numeric(pred_mat[,j]))
  }
  cors
}
...
p_bag <- ggplot(filter(corr_df, Model == sprintf("Bagging (mtry=%d, all features)", ncol(iris_train)-1)),
  ...
  labs(title = sprintf("Bagging (mtry=4)\nMean corr: %.3f",
                       mean(bag_corr[upper.tri(bag_corr)])),
       x = "Tree i", y = "Tree j") +
...
p_rf <- ggplot(filter(corr_df, Model == "Random Forest (mtry=2)"),
  ...
  labs(title = sprintf("Random Forest (mtry=2)\nMean corr: %.3f",
                       mean(rf_corr[upper.tri(rf_corr)])),
...
# RF predictions at different ntree values (predict.all gives individual tree preds)
all_preds <- predict(rf_big, grid_2d, predict.all = TRUE)$individual
# all_preds: nrow(grid_2d) x 500 matrix of class codes (1/2/3)

ntree_snapshots <- c(1, 5, 10, 50, 100, 500)
species_levels  <- levels(iris$Species)

snap_list <- lapply(ntree_snapshots, function(nt) {
  subset_preds <- all_preds[, 1:nt, drop = FALSE]
...
plots_bd <- lapply(ntree_snapshots, function(nt) {
  ...
  acc_nt <- if (nt == 1) {
    mean(predict(single_dt, test_2d, type = "class") == test_2d$Species)
  } else {
    preds_nt <- all_preds[1:nrow(test_2d), , drop = FALSE]
    maj_test <- apply(all_preds[1:nrow(test_2d), 1:nt, drop = FALSE], 1, function(row) {
      tbl <- table(row)
      species_levels[as.integer(names(which.max(tbl)))]
    })
    mean(maj_test == as.character(test_2d$Species))
  }
  ...
  ggplot() +
    ...
    labs(
      title    = if (nt == 1) "Single Tree" else sprintf("%d Trees", nt),
...
# all_test_votes: nrow(iris_test) x 500 matrix (values = 1/2/3 for species)
species_map <- c("1" = "setosa", "2" = "versicolor", "3" = "virginica")
...
vote_plots <- lapply(seq_along(chosen_obs), function(idx) {
  ...
  vote_df   <- as.data.frame(table(votes)) %>%
    mutate(Species = species_map[as.character(votes)],
           Fraction = Freq / 500)
  true_sp   <- as.character(iris_test$Species[i])
  predicted <- species_map[names(which.max(table(votes)))]
  correct   <- (predicted == true_sp)
  conf      <- max(table(votes)) / 500
...
Beyond *how important* features are, we can ask: *in which trees is each feature actually used?* This heatmap shows the **first-split feature** of each tree in the forest — revealing how the random subset mechanism distributes feature usage.

```{r feature-usage-heatmap, fig.height=5, fig.width=10}
set.seed(MASTER_SEED)
n_show_trees <- 60

# For each tree, find the root split variable
root_features <- sapply(1:n_show_trees, function(k) {
  tree_k <- getTree(rf_iris, k = k, labelVar = TRUE)
  root_row <- tree_k[1, ]
  as.character(root_row$`split var`)
})

root_df <- data.frame(
  Tree    = 1:n_show_trees,
  Feature = root_features
)

# Accumulate full feature usage per tree (any split, not just root)
...
ggplot(feature_usage, aes(x = Tree, y = Feature, fill = Count)) +
  ...
  labs(
    ...
    subtitle = "Darker = feature used more splits in that tree. Random subsets → no feature dominates every tree.",
    x = "Tree Index", y = NULL
  ) +
  theme_minimal() +
...
> **Reading the heatmap:** If a column (tree) is uniformly dark, that tree relied heavily on those features. The variation across columns shows that random feature subsetting achieves its goal — different trees learn from different aspects of the data.
